...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        # Multi-GPU setting
        sharded_losses, sharded_losses_change  = parallel.parallel_model(
        ...
        loss_change = tf.add_n(sharded_losses_change) / len(sharded_losses_change)
        loss_change = loss_change + tf.losses.get_regularization_loss()
        ...
        #tf.summary.scalar("loss_change", loss[1])
        tf.summary.scalar("loss", loss)
        tf.summary.scalar("loss_change", loss_change)
        ...
        #    loss[1], colocate_gradients_with_ops=True)
        grads_and_vars = opt.compute_gradients(
            loss_change, colocate_gradients_with_ops=True)
        ...
        train_hooks = [
            ...
            tf.train.NanTensorHook(loss_change),
